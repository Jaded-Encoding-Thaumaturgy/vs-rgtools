...
def RemoveGrainM(clip, mode=2, modeu=None, modev=None, iter=None, planes=None):
    ...
    mode = [0] if iter[0] == 0 else append_params(mode, iter[0])[:iter[0]]
    modeu = [0] if iter[1] == 0 else append_params(modeu, iter[1])[:iter[1]]
    modev = [0] if iter[2] == 0 else append_params(modev, iter[2])[:iter[2]]
...
def Median(clip, radius=None, planes=None, mode='s', vcmode=1, range_in=None, memsize=1048576, opt=0, r=1):
    ...
    mixproc = ('h' in pm or 'v' in pm) and max(
        pr) > 1 and numplanes - len(planes) != 0  # no planes parameter in average.Median
    ...
    if mixproc:
        ...
        return join([
            Median_internal(
                ...
                vcmode[x],
                range_in, [0, 1, 1][x],
                memsize, opt) for x in range(numplanes)])
    ...
    if mode == 's':
        if fmt.sample_type == vs.FLOAT:
            ...
            return core.std.Expr(
                [clip, medu, down, medd],
                ['z a = x y ?' if x in vplanes else '' for x in range(numplanes)])
...
def Blur_internal(clip, radius, aplanes, mode, blur):
    ...
    if 'g' not in blur:
        ...
        return core.std.BoxBlur(
            clip, planes=vplanes, hradius=0 if mode == 'v' else radius, hpasses=1, vradius=0 if mode == 'h' else radius,
            vpasses=1)
    ...
    gmatrix = [
        ...
        [x / 10
         for x in [1, 10, 53, 192, 526, 1148, 2058, 3088, 3923, 4246, 3923, 3088, 2058, 1148, 526, 192, 53, 10, 1]],
        [x / 100
         for x
         in
         [1, 11, 64, 255, 771, 1866, 3732, 6294, 9069, 11257, 12092, 11257, 9069, 6294, 3732, 1866, 771, 255, 64, 11,
          1]],
        [x / 100
         for x
         in
         [1, 12, 76, 330, 1090, 2892, 6369, 11892, 19095, 26620, 32418, 34606, 32418, 26620, 19095, 11892, 6369, 2892,
          1090, 330, 76, 12, 1]],
        [x / 100
         for x
         in
         [1, 13, 89, 418, 1496, 4312, 10351, 21153, 37356, 57607, 78133, 93644, 99442, 93644, 78133, 57607, 37356,
          21153, 10351, 4312, 1496, 418, 89, 13, 1]]]
    bmatrix = [
        [1, 1, 1],
        [1, 2, 3, 2, 1],
        [1, 3, 6, 7, 6, 3, 1],
        ...
        [x / 100
         for x
         in
         [1, 11, 66, 275, 880, 2277, 4917, 9042, 14355, 19855, 24068, 25653, 24068, 19855, 14355, 9042, 4917, 2277, 880,
          275, 66, 11, 1]],
        [x / 100
         for x
         in
         [1, 12, 78, 352, 1221, 3432, 8074, 16236, 28314, 43252, 58278, 69576, 73789, 69576, 58278, 43252, 28314, 16236,
          8074, 3432, 1221, 352, 78, 12, 1]]]
...
def Depth(clip, planes, range_in, cal, outfmt=None):
    ...
    yexpr = ['x 56064 * 4096 +', 'x 65535 *'][range_in] if outfmt is None else [
        'x 4096 - 56064 /', 'x 65535 /'
    ][range_in]
    cexpr = ['x 57344 * 32768 +', 'x 65535 * 32768 +'][range_in] if outfmt is None else[
